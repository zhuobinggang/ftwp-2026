...
def test_row_to_game_state():
    df = read_csv_dataset(inputpath = common.CSV_PATH, split = 'fake_test', suffix = CSV_SUFFIX)
    row = df.iloc[0]
...
def valid_all(model: Model_ucb1, split = 'fake_test_10', game_init_func = GAME_INIT_FUNC):
...
def train_repeat(testing = False, MODEL_INIT_FUNC = Model_ucb1):
    ...
    for rp in range(repeat):
        ...
        for i in range(epoch):
            ...
            score, avg_step = valid_all(model, split=VALID_SPLIT)
            logger.error(f'{get_time_str()} Full valid score ({rp}): {score}, average step {avg_step}')
            logger.warning(f'{get_time_str()} Full valid score ({rp}): {score}, average step {avg_step}')
