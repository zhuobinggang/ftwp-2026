...
def test_trained(repeat = 3):
    ...
    for rp in range(repeat):
        ...
        s2, avg_step = valid_all(model, split='random_10_test', game_init_func=GAME_INIT_FUNC)
        print(f'Full score ({rp}): {s2} {ucb1_on}, average step {avg_step}')
        logger.error(f'Full score ({rp}): {s2} {ucb1_on}, average step {avg_step}')
...
def dataset_info():
    org_train_set = read_csv_dataset(split='train')
    print(f'Original train set size: {len(org_train_set)}')
    move_action_count = 0
    for idx, row in org_train_set.iterrows():
        if row['action'].startswith('go '):
            move_action_count += 1
    print(f'Move action count in original train set: {move_action_count} / {len(org_train_set)} = {move_action_count / len(org_train_set):.2f}')

# 2025.7.24 t-test

def valid_all_keep_individual_scores(model: Model, split = 'random_10_test', game_init_func = None):
    if game_init_func is None:
        game_init_func = GAME_INIT_FUNC
        assert GAME_INIT_FUNC == Game_command_generate_bert_filter_with_navigator, '使用具备navigator的游戏环境'
    game_paths = get_cv_games(split=split)
    scores = []
    steps = []
    logger.debug(f'Validating {split} games, total {len(game_paths)}')
