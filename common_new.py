...
GAME_BASE_PATH = "/home/zhuobinggang/research/datasets/ftwp/cog2019_ftwp/games/"
...
def filter_commands_default(commands, head_words = FILTER_STARTWORD_LIST, command_need_keep = COMMAND_NEED_KEEP):
    ...
    for command in commands:
        if command.startswith('take'): # take apple from fridge -> take apple
            command = re.sub(r'\sfrom.*$', '', command)
        if not any(command.startswith(word) for word in word_list):
            available_actions.append(command)
        else:
            if command in command_need_keep:
                available_actions.append(command)
        # NOTE: 将take apple from fridge变成take apple，去掉from fridge部分，因为我们在生成数据集的时候也会去掉，这样可以保证一致性
